...
Keep GPD-authored auxiliary review artifacts under the selected GPD-owned publication/review roots exposed by centralized preflight. Default project-backed subjects resolve those roots to `GPD/` and `GPD/review`; managed or explicit external publication subjects resolve them to `GPD/publication/{subject_slug}` and `GPD/publication/{subject_slug}/review`. Never write managed-subject review artifacts to the global `GPD/review` fallback. The manuscript itself and any manuscript-local publication manifests stay rooted at the resolved manuscript directory.
