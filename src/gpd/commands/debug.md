...
4. Resolve `DEBUGGER_MODEL=$(gpd resolve-model gpd-debugger)`.
5. Spawn one fresh `gpd-debugger` subagent with the issue summary, evidence, attempted checks, and goal `find_root_cause_or_fix`.
6. Route only on the typed `gpd_return` envelope: present checkpoints to the user, report blocked/failed runs, or read the returned session file for completed diagnoses.
7. Ask whether to apply the fix, continue debugging, or pause with the session file.
