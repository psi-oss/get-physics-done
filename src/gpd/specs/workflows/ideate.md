...
8. Keep deeper checks lightweight and parent-owned. Cheap checks stay inside the normal turn. If a participant reaches the limit of a normal inline turn but can name one bounded deeper check that would materially improve the next pass, announce it briefly in the conversation rather than turning it into a new mode. If the pause will be meaningfully longer than a normal turn, ask inline permission before launching it. No worker waits in place: the parent owns the brief announcement, any permission question, sparse away-status lines, and the fresh detour rerun.
9. When a deeper check launches, keep away-status sparse and semantic. One short launch note is enough on the happy path, with at most one brief progress update if the pause stretches. When the check returns, render the result as a distinct report-back turn from that participant before resuming the normal conversational handoff.
10. End each turn with a lightweight conversational handoff centered on open continuation by default. The workflow-owned priority rule at that handoff is `user interruption > pending agent follow-up > default continuation`. On clean turns, any new user reaction, redirect, setup adjustment, synthesis request, pause, or stop instruction takes priority over any pending follow-up. If the user does not interrupt and no checkpoint, blocker, user-requested narrow follow-up, or worker-announced deeper check needs routing, leave the turn open and ready to continue. Keep those capabilities available in natural language instead of surfacing them as a front-stage menu unless clarity really requires it. If the user wants narrower follow-up after the turn, or a worker surfaced one bounded deeper check, route it through the configuration-adjustment path and run at most one bounded focused fan-out or targeted check before folding the result back into the parent discussion. Raw worker detail remains available only when the user explicitly asks for it.
11. If the turn is ambiguous or a worker returns a checkpoint-worthy blocker, surface that ambiguity in the conversational handoff instead of letting a worker linger or pretending the normal open-continuation default still applies.
...
- pending agent follow-up: if the turn surfaced a checkpoint-worthy blocker, the user explicitly asked for one narrow focused follow-up or targeted check, or a participant announced one bounded deeper check, route that next from the parent handoff with fresh one-shot workers; cheap checks stay inline, and any meaningfully longer pause should ask inline permission before launch; do not leave a worker waiting in place
...
If a worker announced a bounded deeper check, briefly surface it in the handoff. If it is still cheap, keep it inside the normal next turn. If it will take meaningfully longer than a normal turn, ask inline permission before launching it. If launched, keep away-status sparse and make the next visible unit a distinct report-back turn from that participant before returning to the same conversational handoff.
...
If the user asks for a narrower follow-up after a turn, or a worker surfaces one bounded deeper check worth pursuing, route it through the normal adjustment path from the parent handoff. Keep that follow-up parent-owned, bounded, and fileless. Cheap checks stay inline. If the follow-up will take meaningfully longer than a normal turn, ask inline permission before launching it. Run at most one short focused fan-out or targeted check using fresh one-shot workers, keep any away-status sparse, render the result as a distinct report-back turn, then fold it back as a short reintegration note rather than a dedicated subgroup artifact or transcript.
...
- [ ] Agents can briefly announce one bounded deeper check without turning the conversation into a new mode
- [ ] Cheap checks stay inline, while meaningfully longer pauses ask inline permission before launch
- [ ] Deeper-check detours use sparse away-status and return in a distinct report-back turn before the normal handoff resumes
