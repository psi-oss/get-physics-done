...
- If a canonical `result_id` is already known, use `gpd result show "{result_id}"` before dependency tracing. Use `gpd result deps "{result_id}"` for upstream derivations and `gpd result downstream "{result_id}"` for reverse impact.
</step>
