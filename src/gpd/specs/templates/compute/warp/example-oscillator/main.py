...
@wp.kernel
def mse(x_sim: wp.array(dtype=wp.float32), x_obs: wp.array(dtype=wp.float32), n: int, loss: wp.array(dtype=wp.float32)):
    i = wp.tid()
    d = x_sim[i] - x_obs[i]
...
x_obs = wp.array((x_true[obs_idx] + rng.normal(0, 0.02, N_OBS)).astype(np.float32), dtype=wp.float32, device="cpu")
...
for step in range(200):
    ...
    tape.backward(loss)
    wp.synchronize()
    g = sp.grad.numpy().copy().astype(np.float64)
    tape.zero()
    gn = np.linalg.norm(g)
    if gn > 10.0:
        g = g * 10.0 / gn  # gradient clipping for long unrolls
    if step % 25 == 0:
