...
_, la, ta, pa = solve(ty, grad=True)
ta.backward(la)
wp.synchronize()
g_ad = float(pa.grad.numpy()[0])
_, lp, _, _ = solve(ty + 0.1)
_, lm, _, _ = solve(ty - 0.1)
...
for ax, (title, T_plot, b_y) in zip(axes[:2], labels, strict=True):
    im = ax.imshow(
        T_plot.T,
        origin="lower",
        cmap="inferno",
