...
@wp.kernel
def jacobi(T_in: wp.array(dtype=wp.float32), T_out: wp.array(dtype=wp.float32),
           pos: wp.array(dtype=wp.float32), dx: float, n: int):
    i = wp.tid()
    x = float(i) * dx
...
_, la, ta, sa = solve(tp, grad=True); ta.backward(la); wp.synchronize()
g_ad = float(sa.grad.numpy()[0])
_, lp, _, _ = solve(tp + 0.001); _, lm, _, _ = solve(tp - 0.001)
...
Path("/app/output/metrics.json").write_text(json.dumps({
    "answer": round(float(pos[0]), 4), "optimal": 0.5,
