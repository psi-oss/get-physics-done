...
- Surface `deliverables[].kind` (`figure | table | dataset | data | derivation | code | note | report | other`) and `acceptance_tests[].kind` when they affect validation.
- Surface `references[].kind` (`paper | dataset | prior_artifact | spec | user_anchor | other`) and `references[].role` (`definition | benchmark | method | must_consider | background | other`) when they affect validation.
- Link relations use `supports | computes | visualizes | benchmarks | depends_on | evaluated_by | proves | uses_hypothesis | depends_on_lemma | other`; surface them as `links[].relation`.
