...
In explicit-artifact mode, allow one `.tex`, `.md`, `.txt`, `.pdf`, `.docx`, `.csv`, `.tsv`, `.xlsx`, or `.xlsm` review target outside those roots.
...
Workflow-specific intake policy is authoritative: this template defines manuscript-root resolution and manuscript-local artifact gating, but it does not by itself authorize standalone external-subject support for every publication command.
...
Keep GPD-authored auxiliary review, response, and packaging outputs under `GPD/`, but do not treat that as a manuscript-root relocation. The manuscript draft and manuscript-local manifests remain rooted at the resolved manuscript directory.
