...
- `$gpd-execute-phase` orchestrator (automatic post-phase verification via verify-phase.md)
- `$gpd-execute-phase --gaps-only` (re-verification after gap closure)
- `$gpd-verify-work` command (standalone verification on demand)
- `$gpd-regression-check` command (re-verify previously verified truths)
...
Persistent knowledge base of physics error patterns across projects. Stored at `{GPD_INSTALL_DIR}/learned-patterns/`.
...
**Fallback:** If `gpd pattern search` is unavailable, check `{GPD_INSTALL_DIR}/learned-patterns/` directly. If `index.json` exists, filter by domain and read matching patterns.
...
|--------|-----------|--------|
| GREEN  | < 40%     | Proceed normally |
| YELLOW | 40-55%    | Prioritize highest-severity checks, skip optional depth |
| ORANGE | 55-70%    | Complete current check, write partial VERIFICATION.md with checks done so far |
| RED    | > 70%     | STOP immediately, write checkpoint with checks completed so far, mark remaining as "DEFERRED — context pressure", return with CHECKPOINT status |
...
- Subfield-specific priority checks and red flags — load the relevant domain file(s):
  - `@{GPD_INSTALL_DIR}/references/verification-domain-qft.md` — QFT, gauge theory, scattering
  - `@{GPD_INSTALL_DIR}/references/verification-domain-condmat.md` — condensed matter, many-body
  - `@{GPD_INSTALL_DIR}/references/verification-domain-statmech.md` — stat mech, phase transitions
  - `@{GPD_INSTALL_DIR}/references/verification-domain-gr-cosmology.md` — GR, cosmology, black holes, gravitational waves
  - `@{GPD_INSTALL_DIR}/references/verification-domain-amo.md` — atomic physics, quantum optics, cold atoms
  - `@{GPD_INSTALL_DIR}/references/verification-domain-nuclear-particle.md` — nuclear, collider, flavor physics
