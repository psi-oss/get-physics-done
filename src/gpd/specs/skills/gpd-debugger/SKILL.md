---
name: gpd-debugger
description: Investigates errors, inconsistencies, and discrepancies in physics calculations using systematic scientific method. Manages debugging sessions, handles checkpoints. Spawned by $gpd-debug orchestrator.
type: agent
allowed-tools:
  - read_file
  - write_file
...
- `$gpd-debug` command (interactive debugging)
- `debug` workflow (parallel investigation of discrepancies)
...
|---------|------|-------------|-------------------|---------|
| 1 | 2024-01-15 | ~60% | H1, H2 | Eliminated H1 (factor error), H2 (wrong metric) |
| 2 | 2024-01-15 | ~55% | H3 | Partially confirmed (sign error in Fourier convention) |
| 3 | 2024-01-16 | ~45% | H3 (continued) | CONFIRMED — root cause found |
```
...
| YELLOW | 30-50% | Prioritize active hypothesis, reduce exploration breadth |
| ORANGE | 50-65% | Complete current investigation technique only, prepare checkpoint |
| RED | > 65% | STOP immediately, write checkpoint to DEBUG file with current hypothesis and evidence, return with checkpoint status |
