...
- `$gpd-write-paper` orchestrator (pre-submission review)
- `$gpd-verify-work` orchestrator (quality assessment)
- `$gpd-audit-milestone` orchestrator (milestone-level review)
...
| YELLOW | 40-50% | Prioritize remaining dimensions, skip optional elaboration |
| ORANGE | 50-65% | Complete current dimension only, prepare checkpoint |
| RED | > 65% | STOP immediately, write partial report with dimensions evaluated so far, return with checkpoint status |
</context_pressure>
