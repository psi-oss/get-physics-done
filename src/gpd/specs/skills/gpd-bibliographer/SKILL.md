...
- `$gpd-write-paper` orchestrator (bibliography management during paper drafting)
- `$gpd-literature-review` orchestrator (citation network construction)
- `$gpd-execute-phase` (when a phase involves writing or referencing literature)
...
- `@{GPD_INSTALL_DIR}/references/agent-infrastructure.md` -- Shared infrastructure: data boundary, context pressure, external tool failure, return envelope, commit protocol
- `@{GPD_INSTALL_DIR}/references/bibtex-standards.md` -- BibTeX formatting rules, journal abbreviations, arXiv ID formats
</references>
...
```bash
CACHE="$HOME/{GPD_INSTALL_DIR}/verified-references.bib"
if [ -f "$CACHE" ]; then
...
`@{GPD_INSTALL_DIR}/references/bibtex-standards.md`
...
3. **If VERIFIED or CORRECTED:**
   ...
   - Add verification comment:
     ```bibtex
     % Verified: INSPIRE texkey Maldacena:1997re, 2024-01-15
...
**File:** `.planning/references-status.json`
...
| YELLOW | 40-55% | Prioritize remaining verifications, skip optional cross-checks |
| ORANGE | 55-70% | Complete current verification batch only, prepare checkpoint |
| RED | > 70% | STOP immediately, write checkpoint with verifications completed so far, return with checkpoint status |
</context_pressure>
...
- [ ] Appropriate structured return format used (BIBLIOGRAPHY UPDATED / CITATION ISSUES FOUND / CHECKPOINT)
      </success_criteria>
