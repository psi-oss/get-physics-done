...
Tiers are resolved to platform-specific models at runtime by gpd CLI.
On single-model platforms, all tiers resolve to null (use the platform's default model).
...
| Profile           | Behavioral Change                                                                                                                                                                                                                                                                                    |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **deep-theory**   | Full verification. Runs ALL 15 physics verification checks (5.1 dimensional analysis, 5.2 numerical spot-checks, 5.3 limiting cases, 5.4 independent cross-checks, 5.5 intermediate result spot-checks, 5.6 symmetry verification, 5.7 conservation laws, 5.8 mathematical consistency, 5.9 convergence testing, 5.10 literature comparison, 5.11 order-of-magnitude plausibility, 5.12 statistical rigor, 5.13 thermodynamic consistency, 5.14 spectral structure, 5.15 anomalies/topology). Requires INDEPENDENTLY CONFIRMED confidence for every key derivation result. Re-derives every limiting case. Full dimensional analysis trace. No shortcuts.                                                                                |
| **numerical**     | Computation-focused verification. Emphasizes convergence testing (5.9), numerical spot-checks (5.2), error budgets, code validation. De-emphasizes analytical re-derivation unless it validates numerics. Runs all numerical checks at 3+ resolution levels.                                         |
| **exploratory**   | Lightweight verification. Runs 7-check floor: dimensional analysis (5.1), numerical spot-checks (5.2), limiting cases (5.3), symmetry (5.6), conservation (5.7), math consistency (5.8), and literature agreement (5.10). Skips convergence, thermodynamic consistency, spectral structure, and anomalies. Catches gross errors without blocking exploration. |
| **review**        | Cross-validation focused. Runs ALL checks. Additionally: compares every numerical result against at least 2 literature values. Verifies every approximation is justified with explicit bounds. Checks that error bars are conservative. Flags any result that cannot be cross-validated.             |
| **paper-writing** | Publication-readiness verification. Runs all checks. Additionally verifies: figures match data, equations in text match derivation files, notation is consistent throughout, all symbols are defined, references exist.                                                                              |
...
| **exploratory**   | Reduces verification to limiting cases + dimensional analysis only. Allows larger tasks (up to 90 min). Accepts 3-4 tasks per plan. Rapid coverage over exhaustive proof.                                                 |
...
| **exploratory**   | Speed over depth. Larger tasks (up to 90 min). Skips intermediate verification checkpoints — verifies only final results. Accepts approximate answers to move through parameter space quickly. Flags uncertainties for later. |
