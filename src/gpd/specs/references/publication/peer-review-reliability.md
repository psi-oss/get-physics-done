...
This is the canonical reliability reference for the peer-review skill surface. Follow the path and round-suffix conventions here when the workflow, report, and response artifacts need a stable source of truth. `gpd:peer-review` is project-aware: it can review the active manuscript in the current GPD project or an explicit `.tex`, `.md`, `.txt`, `.pdf`, or manuscript-directory target, while writing review artifacts under the `selected_publication_root` and `selected_review_root` surfaced by centralized preflight. Default project-backed subjects resolve those selected roots to `GPD/` and `GPD/review`; managed or explicit external publication subjects resolve them to `GPD/publication/{subject_slug}` and `GPD/publication/{subject_slug}/review`. Do not write a managed-subject review bundle to global `GPD/review` as a fallback or duplicate copy.
