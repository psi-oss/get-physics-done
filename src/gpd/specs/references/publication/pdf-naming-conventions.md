...
- `gpd.mcp.paper.compiler.build_paper` -- Calls `derive_output_filename` to name the `.tex` file; the compiler then produces a `.pdf` with a matching stem.
...
- When `output_filename` is provided, no sanitization is applied -- the caller is responsible for choosing a valid name.
