...
```bash
gpd commit "docs: initialize [project-name] ([N] phases)" --files .planning/
```
...
```bash
gpd commit "docs({phase}-{plan}): complete [plan-name] plan" --files ${phase_dir}/{phase}-{plan}-PLAN.md ${phase_dir}/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
gpd commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/
```
