...
For a context-efficient matrix mapping each error class to the checks most likely to catch it, load `references/verification/errors/llm-errors-traceability.md`. That companion index covers dimensional analysis, limiting cases, symmetry, conservation, Ward/sum-rule checks, numerical convergence, literature cross-checks, and positivity/unitarity against all 104 classes.
...
6. **Use the traceability matrix.** When a specific error class is suspected, consult `references/verification/errors/llm-errors-traceability.md` to identify which verification checks are most effective for detection.
