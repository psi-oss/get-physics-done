...
**Related:** For per-agent pressure thresholds (GREEN/YELLOW/ORANGE/RED), see `context-pressure-thresholds.md`. For the canonical threshold table, see `agent-infrastructure.md` §Context Pressure Management.
  </overview>
...
**Subagent advantage:** Each subagent (gpd-executor, gpd-planner, etc.) gets a fresh context window. The orchestrator's context stays lean. This is why execute-plan.md routes autonomous work to subagents.
</context_consumption>
