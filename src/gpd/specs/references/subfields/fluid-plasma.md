...
**Detailed protocols:** For step-by-step calculation protocols, see `protocols/fluid-dynamics-mhd.md` (regime identification, governing equations, turbulence, MHD checks), `protocols/numerical-computation.md` (CFD methods, convergence testing), `protocols/stochastic-processes.md` (turbulence models, Langevin approaches), `protocols/non-equilibrium-transport.md` (transport coefficients, Kubo formulae for viscosity and conductivity), `protocols/molecular-dynamics.md` (particle-in-cell methods, kinetic simulations), `protocols/order-of-limits.md` (ideal vs resistive limits, incompressible limit), `protocols/kinetic-theory.md` (Boltzmann equation, Chapman-Enskog, transport coefficients from first principles). For verification formulas: `verification-domain-fluid-plasma.md` (MHD equilibrium, Alfven waves, reconnection, conservation laws).
