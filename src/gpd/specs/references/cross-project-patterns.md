...
Seed patterns derived from common LLM physics error classes (see `llm-physics-errors.md`). These provide immediate value even before the automated library infrastructure is in place.
...
**Prevention:** Use the systematic procedure: identify mass dimension in natural units, then restore [energy] = [M][c²], [length] = [ℏ]/([M][c]), [time] = [ℏ]/([M][c²]). See error class 33 in llm-physics-errors.md for worked examples.
