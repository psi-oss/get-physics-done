...
|---|---|
| **supervised/guided** | Present candidate search strategies before executing. Checkpoint after each search round with findings summary. Ask user to confirm scope boundaries and relevance criteria. |
| **autonomous** | Execute full search strategy independently. Make scope judgments without asking (include/exclude borderline papers based on citation network evidence). Produce complete LITERATURE-REVIEW.md without checkpoints. |
