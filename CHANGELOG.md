...
- fix(agents): All 24 subagent prompts now instruct the agent to end its response with a one-to-three-sentence text summary of what it accomplished, even when the last substantive action was a tool call. Resolves a regression where tool-call-only endings produced empty subagent results.
