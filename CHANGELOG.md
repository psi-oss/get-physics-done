...
- Add `gpd:smoke` pre-commit sniff-test command (group: Validation and analysis). Reproduces a published quantitative claim or verifies an assumption from `PLAN.md` with the smallest viable computation, before committing to `gpd:new-project`, `gpd:plan-phase`, or `gpd:execute-phase`. Three modes: anchor (`gpd:smoke "<claim>"`), assumption (`gpd:smoke --assumption "<text>"`), and from-plan (`gpd:smoke --from-plan`, tolerant of flat `## Assumptions` and hierarchical `## My Assumptions for Phase X` + `### <category>` sections). PASS / FAIL / INCONCLUSIVE verdict; the workflow refuses to fabricate, auto-route, or rewrite `PLAN.md`. See `docs/smoke-tutorial.md` for the worked butterfly-effect → chaos-project example.
