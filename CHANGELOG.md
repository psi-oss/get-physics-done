...
- Fix `gpd suggest` ignoring actual project state: use `_project_scoped_cwd()` so project-scoped reads resolve the verified project root without mutating legacy root planning docs.
- Fix `gpd query search` scope and phase-range correctness: mixed queries that combine SUMMARY frontmatter filters (`provides` / `requires` / `affects`) with scoped content filters (`text` / `equation`) now keep honoring `--scope` instead of collapsing to SUMMARY-only search, and bounded `--phase-range` filtering now applies consistently to registry-backed matches, excluding blank/unphased results.
...
- Auto-migrate `ROADMAP.md` and `PROJECT.md` from workspace root into `GPD/` only on explicit write/repair surfaces such as `gpd health --fix`, while read-only CLI and MCP paths remain non-mutating.
