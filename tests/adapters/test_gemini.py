...
def expected_gemini_launcher(target: Path) -> str:
    return json.dumps(str((target / "bin" / "gpd").resolve()))


def expected_gemini_launcher_unquoted(target: Path) -> str:
    return str((target / "bin" / "gpd").resolve())
...
class TestInstall:
    ...
    def test_install_enables_experimental_agents(self, adapter: GeminiAdapter, gpd_root: Path, tmp_path: Path) -> None:
        ...
        assert sorted(manifest["managed_runtime_files"]) == ["bin/gpd", "policies/gpd-auto-edit.toml"]
    ...
    def test_install_adds_policy_path_shell_sentinel_and_policy_file(
        ...
        assert str((target / "bin" / "gpd").resolve()) in policy
        assert "'" + expected_gemini_launcher_unquoted(target).replace("'", "''") + "'" in policy
    ...
    def test_install_creates_pinned_launcher_and_rewrites_gpd_cli_calls(
        ...
        expected_launcher = expected_gemini_launcher(target)
        ...
        assert f"When shell steps call the GPD CLI, use {expected_launcher}" in command
        ...
        assert f'INIT=$({expected_launcher} init new-project)' not in workflow
        assert f"{expected_launcher} init new-project" in workflow
        assert f"{expected_launcher} commit " in workflow
        assert ' gpd commit "' not in workflow
        assert f"{expected_launcher} --raw validate project-contract {_GEMINI_APPROVED_CONTRACT_PATH}" in command
        assert f"{expected_launcher} state set-project-contract {_GEMINI_APPROVED_CONTRACT_PATH}" in command
        ...
        assert f"{expected_launcher} --raw validate project-contract {_GEMINI_APPROVED_CONTRACT_PATH}" in workflow
        assert f"{expected_launcher} state set-project-contract {_GEMINI_APPROVED_CONTRACT_PATH}" in workflow
        assert f"{expected_launcher} --raw validate project-contract {_GEMINI_APPROVED_CONTRACT_PATH}" in state_schema
        assert f"{expected_launcher} state set-project-contract {_GEMINI_APPROVED_CONTRACT_PATH}" in state_schema
    ...
    def test_install_rewrites_set_profile_shell_block_for_gemini(
        ...
        assert expected_gemini_launcher(target) + " config ensure-section" in content
        assert expected_gemini_launcher(target) + " init progress --include state,config" in content
