...
class TestBlackboardServer:
    """Tests for gpd.mcp.servers.blackboard_server tool functions."""

    @pytest.fixture(autouse=True)
    def _mock_store(self):
        """Provide a mock BlackboardStore and inject it into the module."""
        self.store = MagicMock()
        with patch("gpd.mcp.servers.blackboard_server._store", None):
            with patch("gpd.mcp.servers.blackboard_server._get_store", return_value=self.store):
                yield

    def test_search_blackboard_returns_entries(self):
        from gpd.mcp.servers.blackboard_server import search_blackboard

        self.store.query_nodes.return_value = [{"id": "n1", "label": "energy"}]
        result = search_blackboard("energy")
        assert result["query"] == "energy"
        assert result["match_count"] == 1
        assert len(result["entries"]) == 1

    def test_search_blackboard_with_filters(self):
        from gpd.mcp.servers.blackboard_server import search_blackboard

        self.store.query_nodes.return_value = []
        result = search_blackboard("mass", node_types=["equation"], tags=["qft"], limit=5)
        self.store.query_nodes.assert_called_once_with(
            node_types=["equation"], tags=["qft"], text_query="mass", limit=5
        )
        assert result["match_count"] == 0

    def test_get_entry_found(self):
        from gpd.mcp.servers.blackboard_server import get_entry

        self.store.get_node.return_value = {"id": "n1", "label": "E=mc^2"}
        self.store.get_edges.side_effect = [
            [{"edge_id": "e1", "src": "n1", "dst": "n2", "edge_type": "derives_from"}],
            [],
        ]
        result = get_entry("n1")
        assert result["entry"]["label"] == "E=mc^2"
    ...
    def test_get_entry_not_found(self):
        ...
        result = get_entry("missing")
        assert "error" in result

    def test_get_subgraph_traversal(self):
        from gpd.mcp.servers.blackboard_server import get_subgraph

        self.store.get_node.side_effect = lambda nid: {"id": nid, "label": f"node-{nid}"}
        self.store.get_edges.return_value = [{"edge_id": "e1", "src": "root", "dst": "child", "edge_type": "supports"}]
        result = get_subgraph("root", depth=1)
    ...
    def test_get_subgraph_edge_type_filter(self):
        ...
        self.store.get_edges.return_value = [
            {"edge_id": "e1", "src": "root", "dst": "c1", "edge_type": "supports"},
            {"edge_id": "e2", "src": "root", "dst": "c2", "edge_type": "contradicts"},
        ]
        result = get_subgraph("root", depth=1, edge_types=["supports"])
        assert result["edge_count"] == 1

    def test_find_by_type(self):
        from gpd.mcp.servers.blackboard_server import find_by_type

        self.store.query_nodes.return_value = [{"id": "n1"}]
        result = find_by_type("equation", limit=10)
        assert result["node_type"] == "equation"
        assert result["match_count"] == 1

    def test_find_related(self):
        from gpd.mcp.servers.blackboard_server import find_related

        self.store.get_node.side_effect = lambda nid: {"id": nid, "label": f"node-{nid}"}
        self.store.get_edges.return_value = [
            {"edge_id": "e1", "src": "n1", "dst": "n2", "edge_type": "supports"},
            {"edge_id": "e2", "src": "n1", "dst": "n3", "edge_type": "other"},
        ]
        result = find_related("n1", "supports")
    ...
    def test_find_related_not_found(self):
        ...
        self.store.get_node.return_value = None
        result = find_related("missing", "supports")
        assert "error" in result

    def test_propose_entry(self):
        from gpd.mcp.servers.blackboard_server import propose_entry

        self.store.get_node.return_value = {"id": "ev1"}
    ...
    def test_propose_entry_no_evidence(self):
        from gpd.mcp.servers.blackboard_server import propose_entry

        result = propose_entry(kind="assumption", label="A1", content="test")
        assert result["evidence_linked"] == 0

    def test_list_entry_types(self):
        from gpd.mcp.servers.blackboard_server import list_entry_types

        self.store.count_by_type.return_value = {"equation": 5, "result": 3}
        result = list_entry_types()
        assert result["type_count"] == 2
        assert result["total_entries"] == 8

    def test_get_coverage(self):
        from gpd.mcp.servers.blackboard_server import get_coverage

        self.store.get_node.side_effect = lambda eid: {"id": eid, "label": f"node-{eid}", "confidence": 0.9}
        self.store.get_edges.side_effect = lambda eid, direction: (
            [
        ...
        result = get_coverage(["n1", "n2"])
        assert result["total_checked"] == 2
        assert result["well_supported"] == 2

    def test_get_coverage_missing_entry(self):
        from gpd.mcp.servers.blackboard_server import get_coverage

        self.store.get_node.return_value = None
        result = get_coverage(["missing"])
...
# ---------------------------------------------------------------------------
# Blackboard _get_store error path
# ---------------------------------------------------------------------------


class TestBlackboardStoreInit:
    """Test blackboard store initialization error handling."""

    def test_get_store_missing_env_var(self):
        import gpd.mcp.servers.blackboard_server as bb_mod
        from gpd.mcp.servers.blackboard_server import _get_store

        bb_mod._store = None
        with patch.dict("os.environ", {}, clear=True):
            with pytest.raises(RuntimeError, match="BLACKBOARD_DB_PATH"):
                _get_store()
        bb_mod._store = None
