...
def test_write_paper_bootstrap_contract_is_explicit_about_project_managed_manuscript_lane() -> None:
    source = WORKFLOW.read_text(encoding="utf-8")
    ...
    assert "The fresh bootstrap root may be the legacy `paper/` scaffold or the managed project lane `GPD/publication/{subject_slug}/manuscript`" in source
    assert "a resolved `${PAPER_DIR}` under `GPD/publication/{subject_slug}/manuscript` is a project-managed manuscript lane" in source
    assert "do not invent an external-manuscript `write-paper` flow or relax the project-required contract" in source
