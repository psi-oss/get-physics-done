...
WORKFLOW_EXEMPT_COMMANDS = frozenset({"export-chat", "health", "suggest-next"})
...
def test_commands_are_workflow_backed_or_explicitly_exempt() -> None:
    ...
    for command_stem in sorted(WORKFLOW_EXEMPT_COMMANDS):
        ...
        if command_stem == "export-chat":
            assert "gpd export-chat" in command_text
            assert "@{GPD_INSTALL_DIR}/workflows/export-chat.md" not in command_text
