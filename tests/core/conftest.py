...
def planning_dir(tmp_path: Path) -> Path:
    """Create a mock .gpd/ directory with required structure.

    Returns the project root (tmp_path), not .gpd/ itself.
...
def sample_phase_dir(planning_dir: Path) -> Path:
    ...
    """
    phase = planning_dir / ".gpd" / "phases" / "01-setup"
...
def mock_roadmap(planning_dir: Path) -> Path:
    ...
    """
    roadmap = planning_dir / ".gpd" / "ROADMAP.md"
    roadmap.write_text(
        "# Roadmap\n\n"
    ...
    phases_dir = planning_dir / ".gpd" / "phases"
