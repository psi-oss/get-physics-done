...
def test_verification_report_strict_pass_mentions_required_reference_coverage() -> None:
    ...
    assert "If decisive work remains open, use `gaps_found`, `expert_needed`, or `human_needed`" in verification_report
    assert "Nested `contract_results` entries still use the canonical contract-result status vocabulary" in verification_report
    assert "including `partial` when a specific claim, deliverable, or acceptance test is only partly satisfied" in verification_report
