...
**6c. Append quick task table row (apply_patch + sync):**

If the "Quick Tasks Completed" table section in STATE.md does not exist, create it using apply_patch tool after the `### Blockers/Concerns` section:
...
Append new row to the table using apply_patch tool:

```markdown
| ${next_num} | ${DESCRIPTION} | ${date} | ${commit_hash} | [${next_num}-${slug}](./quick/${next_num}-${slug}/) |
```

**6d. Force state.json re-sync after the apply_patch:**

```bash
gpd state load
```

This ensures the table content added via apply_patch is synced to state.json. Do NOT skip this step — direct apply_patch of STATE.md without `state load` causes STATE.md/state.json divergence.
