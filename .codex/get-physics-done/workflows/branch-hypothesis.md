...
2. Add hypothesis metadata to STATE.md via apply_patch tool. After the `## Current Position` section, add:
...
3. Force state.json re-sync after the apply_patch:

```bash
gpd state load
```

The `state load` ensures state.json reflects the apply_patch tool changes. Do NOT skip this step.
   </step>
