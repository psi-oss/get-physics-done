...
1. If the reverted commit added a phase completion marker, remove it via apply_patch tool
2. If the reverted commit advanced the current phase, roll back the position via apply_patch tool
...
4. If any direct apply_patch was needed (steps 1-2), force state.json re-sync:
